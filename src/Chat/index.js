...
const examples = [
  'Explain superconductors like Im five years old',
  'Design a database schema',
  'Give me ideas how start coding',
...
const Chat = () => {
  ...
  // rhr='http://localhost:8000/api/chat';
  // const apiKey = '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';
  const apiKey = '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';
  const handleSend = async () => {
    if (input.trim) {
      ...
      if (!title) {
        const createTitle = await fetch('https://api.openai.com/v1/chat/completions', {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${apiKey}`
      ...
      <div className=' w-[80%]'>
          ...
          chat.length > 0 ?
            ...
            (
              <div className=' h-[80%] flex flex-col justify-center items-center text-white'>
                <div className=' text-4xl font-bold mb-8'>Chat SOMYKORON</div>
                <div className=' text-1xl font-bold mb-8'>how can i help you today?</div>
        ...
        <div className=' h-[20%]'>
          <div className=' flex flex-col items-center justify-center w-full h-full text-white'>
            ...
            <small className=' text-slate-500 mt-2'>ChatGPT can make mistakes. Consider checking important information.</small>
          </div>
        </div>
      </div>
    </div>
